...
- **Scope discovery** — `list_scopes` enumerates distinct tag values by grouping an **in-scope metric** (the first entry of the resolved registry) by tag. `env` is Datadog's standard environment tag; **tenant is org-specific**, so the tag key is configurable via `DATADOG_TENANT_TAG` (default `tenant`). Normally bypassed entirely: a `COPILOT_PLATFORM_ENVIRONMENTS`/`_TENANTS` list is served directly. Validated only against mocked HTTP so far (see OPEN-QUESTIONS). The old `DATADOG_DISCOVERY_METRIC` setting was removed 2026-08-05.
- **Metric namespace / metric scope** — `DATADOG_METRIC_NAMESPACES` (e.g. `ec.*, ea.*`): comma-separated wildcard patterns that form an **allowlist** of the only metrics the copilot may read. Parsed by `app/telemetry/namespaces.py`; empty means no filtering. Applied to live discovery, Terraform extraction, and the registry merge alike. Distinct from **`COPILOT_PLATFORM_METRICS`**, which is only guard vocabulary and filters nothing.
- **Metric registry** — the `{metric_name: datadog_query}` map the adapter can query, assembled by `copilot.merged_metric_queries` with precedence **`DATADOG_METRIC_QUERIES` > Terraform-extracted > live-discovered**, then namespace-filtered. Distinct from the per-question **selection** (`resolver.select_metrics`, top-8) which is what actually gets fetched.
