...
- **Wire the guard's Stage-2 LLM classifier** (a cheap relevance call for the ambiguous middle; today hybrid mode refuses the middle since no classifier is wired). Iteration 3's platform-vocabulary guard extension (below) reduces how often this gap is actually hit for the one platform in scope, but doesn't replace it — still deferred.
- **Fix `.env` not loading at runtime** (blocks *live* Datadog scope discovery; build/tests unaffected — mocked).

## Iteration 3 — platform-narrowed scope + `@` scope menu (in progress, 2026-07-27)
Approved feature: stop forcing environment/tenant/duration selection before every question, by narrowing the Copilot to one known platform (few environments/tenants, known metric/log/trace names) instead. Two sub-phases, backend first so the harder UI risk (composer rewrite) is scoped in isolation.

- [x] **Phase 1 (backend) — done.** `COPILOT_PLATFORM_*` config (`.env.example`/`app/config.py`): environments, tenants, metrics, log sources, trace services, default window (days) — all optional, blank by default. `Scope.validation_error()` relaxed: environments/tenants and duration are no longer mandatory (only an internally-inconsistent partial duration or an over-7-day span is rejected); an unset field is backfilled downstream, never rejected upfront. `Copilot._with_defaults()` fills any missing scope field from the platform config (or a bare 2-day unfiltered window if unconfigured) and persists the resolved scope — so a question can be asked with zero scope interaction. `guard.evaluate()` gained `extra_vocabulary`, sourced from the platform's metrics/log-sources/trace-services/tenants/environments, recognized in addition to the generic on-topic word list. `Copilot.list_scopes()`/`/api/scopes` serve the static platform config directly (bypassing live Datadog discovery) once configured. `scripts/check_env.py` reports what the platform config resolved to. 274/275 green (+13 specs), 98% coverage. See DECISIONS 2026-07-27.
- [x] **Phase 2 (UI) — done.** Removed `#scope-trigger`/`#scope-panel`/the hidden duration `<select>`. Composer (`#input`) rebuilt as `contenteditable` (a plain `<textarea>` can't host an inline non-text widget). Typing `@` opens a caret-anchored `#at-menu` popover (Environment/Tenant/Duration → known values from `/api/scopes`, Duration reusing the existing presets + custom-range modal); a selection inserts a locked, non-editable chip (`contenteditable="false"`, explicit "✕" remove control) — never parsed back out of free text. `composerText()` builds the message by skipping chip nodes; `scopePayload()` builds the scope by reading chips, returning `undefined` when none are present so a plain follow-up reuses the conversation's existing/default scope rather than resetting it every turn. Persona ("Explain as") stays a separate, now plainly-visible `<select>`, untouched by this change. Send no longer gated on any scope selection. UI-contract tests (`tests/test_web_ui.py`) and the Playwright smoke test rewritten for the new contract; 279/280 green. **Real-browser verification attempted but blocked in this sandbox** (Chromium's shared-library deps require root `apt` install, unavailable here) — the interactive caret-menu/chip behavior needs a manual check in an actual browser (or a machine where `playwright install-deps` can run) before this ships. See DECISIONS 2026-07-27.
- [ ] Fill in real `COPILOT_PLATFORM_*` values once on the company laptop with Datadog access (README has exactly where to look for each).
- [ ] Manually verify the `@` menu (caret positioning, chip insertion/removal, custom-range modal) in a real browser — not yet done (see Phase 2 note above).
