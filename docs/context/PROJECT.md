...
Four separable layers — **telemetry** (DataSource: ReplayAdapter + read-only LiveDatadogAdapter), **reasoning** (structured Fact/Hypothesis/Recommendation/Unknown + timeline + evidence grounding), **Workspace** (SQLite append-with-history), **presentation** (conversation sidebar · chat · live Workspace panel). Plus: multi-conversation memory; a registry of 5 personas and the Incident Summary artifact; a pre-reasoning **relevance & abuse guard**; **scoped investigations** — a per-conversation `Scope` (environments/tenants/time window, ≤7 days) selected by typing `@` in the composer (which inserts locked chips) and threaded into the data queries; a **correlation layer** over the `ec-conduct-dd-monitors` Terraform repo; and **metric scope** — one `DATADOG_METRIC_NAMESPACES` allowlist (e.g. `ec.*`) that decides which metrics exist for the copilot at all, resolved per question down to the top-8 relevant ones. A separate `metrics/` subsystem audits the build. See `STATE.md` for the live status and `ROADMAP.md` for what's next.

**Running live on real Datadog works** (verified 2026-08-05: 420-metric registry, real evidence-backed investigations). Both scope tag keys are org-specific and configurable — on the EC org the tenant tag is `tenant` and the *environment* tag is `kube_namespace` (there is no `env` tag), and its metrics are sparse enough to need a 7-day window. See `STATE.md` for the exact working config before touching live settings.
...
```
Then open http://127.0.0.1:8000 . The app starts without a key (chat reports it isn't configured); add `ANTHROPIC_API_KEY` **or** just be signed in to the local `claude` CLI (keyless) to enable real reasoning. Verified booting via smoke test (2026-06-26).

For the **live Datadog** source, `.env` also needs `COPILOT_DATA_SOURCE=datadog`, a credential (a PAT is enough), `DATADOG_METRIC_NAMESPACES`, both tag keys (`DATADOG_TENANT_TAG` / `DATADOG_ENV_TAG`), and a `DATADOG_CA_BUNDLE` on a TLS-inspecting corporate network. Two safe diagnostics exist and are worth running before debugging anything: `python scripts/check_env.py` (offline; prints what resolved, never secret values) and `python scripts/datadog_probe.py` (read-only; prints what your org actually returns). Note `.env` does **not** override real environment variables — a stray `export` in your shell wins, so check `env | grep DATADOG` if resolved config disagrees with the file.
