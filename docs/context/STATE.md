...
**Control-row refinement (2026-07-08):** slimmed the scope pills to a single tidy row (dropped the stacked uppercase labels — each pill self-describes, e.g. `Environment: prod`); env/tenant filter box only appears when a list exceeds 8 options; Duration is a plain dropdown and "Custom range…" now opens a **modal** with From/To pickers constrained to not-future and within the last 2 years (≤7-day span). Native selects use a custom SVG chevron for the Claude look.
