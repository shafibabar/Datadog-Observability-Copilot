...
**Control-row refinement (2026-07-08):** slimmed the scope pills to a single tidy row; env/tenant filter box only appears when a list exceeds 8 options; Duration is a plain dropdown and "Custom range…" opens a **modal** with From/To pickers constrained to not-future and within the last 2 years (≤7-day span).
**Scope menu redesign (2026-07-08, Claude model-picker style):** replaced the four separate pills with ONE `#scope-trigger` pill (shows a live summary, e.g. `prod · acme · 1h · SRE`) opening a `#scope-panel` popover with **drill-down submenus** — root rows `Environment / Tenant / Duration / Explain as` (each shows its current value + `›`), drilling into option lists (multi-select checkmarks for env/tenant with a filter box past 8 options; single-select presets for Duration; personas with title+description+checkmark for Explain as, like the model list). Hidden `<select id="duration-select">`/`<select id="persona">` remain the state holders the menu drives (keeps logic + tests stable). UI-contract tests updated in `tests/test_web_ui.py`.
**Fix (2026-07-08):** drilling into a submenu closed the whole menu (empty Environment/Tenant/Duration/Explain-as) — the rebuilt panel detached the clicked row, so the bubbled click hit the outside-close handler. Fixed by stopping propagation on `#scope-panel`. Also added an **opt-in Playwright browser smoke test** (`tests/test_smoke_playwright.py`) that drives a real browser to open the menu + drill into Duration/Explain-as; SKIPPED unless `playwright` + a browser are installed (offline suite stays green: 243 passed, 1 skipped). Enable with `pip install playwright && playwright install chromium`. Playwright is intentionally NOT added to `requirements-dev.txt` (new dep needs approval + network).
