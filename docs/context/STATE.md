...
## Current gate
Plan + Design **approved**. **Iterations 0–2 COMPLETE.** Two independent workstreams have since landed and merged: **Iteration 3 (platform-narrowed scope + `@` scope menu)** and the **monitors/correlation layer** (Terraform-extracted EC metrics, wired Stage-2 guard classifier). Both are code-complete and green — see their sections below and DECISIONS for the full narrative. **Combined pending items before going live:**
1. **Live Datadog validation on the work laptop** — set `COPILOT_DATA_SOURCE=datadog`, a Datadog credential, `DATADOG_TENANT_TAG=kube_namespace`, `MONITORS_REPO_PATH=<checkout>`, and the `COPILOT_PLATFORM_*` values (README has exactly where to find each in the Datadog UI), then run `scripts/check_env.py`/`scripts/datadog_probe.py`.
2. **Manually verify the new `@` scope menu in a real browser** — this sandbox couldn't install Chromium's system deps (needs root `apt`), so the caret-anchored popover/chip behavior is covered only by static contract tests, not an actual render.
...
## Tests
**299 passing, 1 skipped (Playwright, needs a browser) — 100% of runnable, 98% coverage.** No pending (red) specs. This is Iteration 3 (platform-scope config + guard vocabulary + relaxed Scope validation, then the `@`-menu composer rewrite) merged together with the monitors/correlation-layer branch (Terraform-extracted metrics, wired Stage-2 guard classifier) — both landed cleanly with only two textual merge conflicts (`app/copilot.py`'s `build_copilot` wiring, and this file + `DECISIONS.md`), no semantic clashes; the full combined suite is green. Full per-step log in `TESTING.md`.
