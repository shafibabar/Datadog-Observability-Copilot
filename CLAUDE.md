...
- **`scripts/`** — safe, human-runnable diagnostics: `check_env.py` (reports which `.env`/config keys are set — never their values — and what `Settings` resolves to) and `datadog_probe.py` (makes read-only calls against a real Datadog org and prints request/response shapes, to tune `app/telemetry/datadog.py`).
