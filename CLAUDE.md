...
- **`app/monitors/`** — the **correlation layer** over a local checkout of the Terraform repo `ec-conduct-dd-monitors`, located via **`MONITORS_REPO_PATH`** (machine-specific; empty/missing → empty index, graceful). `index.py` extracts monitors, dashboards, a normalized `ec.*` **metric-query map** (~320 on the real repo; merged into the Datadog adapter's registry via `copilot.merged_metric_queries`, precedence `DATADOG_METRIC_QUERIES` > extracted > infra defaults) and a vocabulary **alias map** ("message processing" → its metrics). `resolver.py` deterministically selects the top-8 question-relevant metrics per ask (alias/token matching, golden-set fallback); `build_evidence_catalog(metrics=…)` queries only that selection — a large registry must never mean query-everything. The monitors context is injected into **every** reasoning prompt when non-empty (deliberately not keyword-gated). EC's "tenant" is `kube_namespace` → set `DATADOG_TENANT_TAG=kube_namespace`. Tests use fixture Terraform trees in `tmp_path`, never the real checkout. See `docs/MONITORS_INTEGRATION.md`.
